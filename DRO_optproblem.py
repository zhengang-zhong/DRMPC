...
class Opt_problem:
    def __init__(self, model, Q, Qf, R, beta = 0.95, beta_prob = 0.95, N_sample = 5, i_th_state = 1, i_state_ub = 0.05, epsilon = 1, sin_const = 1, collect = None, est = False, data_set = None, mu = None, sigma = None):
        ...
        if not collect:
            ...
            lambda_var, gamma_matrix, theta_matrix, si_var, constraint = self.define_constraint(W_sample_matrix, W_sample_matrix_ext, eigvec, H_cal_dec, H, H_new, n, d, Bx, Cx_tilde, Cy_tilde,
                              Ey_tilde, N, N_sample, sin_const, i_th_state, i_state_ub, epsilon,beta_prob)
        if collect:
    ...
    def define_constraint(self, W_sample_matrix, W_sample_matrix_ext, eigvec, H_cal_dec, H, H_new, n, d, Bx, Cx_tilde, Cy_tilde,
        ...
        for i in range(N_sample):
            for j in range(N):
                # theta_var = cp.Variable((theta_shape, 1), nonneg=True)
                theta_var = cp.Variable(1,nonneg=True)
                #             gamma_var = cp.Variable([gamma_shape,1])
                theta_matrix += [theta_var]
        # theta_matrix = cp.Variable(N_sample*N,nonneg=True)
        # k in N, i in N_sample
        # bk - <ak,xi_i>
        X_constraint = Bx @ H_cal_dec @ (Cy_tilde + Ey_tilde) + Cx_tilde
        ub_list = [i_state_ub] * (N+1)
        # ub_col = np.c_[ub_list]
        #     si_var = cp.Variable((N_sample,1))
        si_var = cp.Variable(N_sample,nonneg=True)
        A_constriant = X_constraint[:,1:]
        B_constriant = X_constraint[:, [0]]

        for i in range(N_sample):
            for j in range(N):
                #             print(N_sample)
                constraint_temp = 1 - theta_matrix[i * N + j] * ( (ub_list[j]-B_constriant[n * (j + 1) + i_th_state]) -
                                                                 A_constriant[[n * (j + 1) + i_th_state],:]@
                                                                 W_sample_matrix[:, [i]]) + gamma_matrix[i * N + j].T \
                                  @ (d_supp - C_supp @ W_sample_matrix[:, [i]])

                constraint += [constraint_temp <= si_var[i]]
        # print("theta_var",theta_var.shape)
        ...
        for i in range(N_sample):
            for j in range(N):
                ...
                # constraint_temp = - C_supp.T @ gamma_matrix[i * N + j] + theta_matrix[i * N + j] * ak_matrix[[n * (
                #             j + 1) + i_th_state], :].T
                constraint_temp = - C_supp.T @ gamma_matrix[i * N + j] + theta_matrix[i * N + j] * ak_matrix[[n * (
                            j + 1) + i_th_state], :].T
        ...
        constraint += [lambda_var * epsilon + 1 / N_sample * cp.sum(si_var) <= beta_prob]
        # constraint += [lambda_var * epsilon + 1 / N_sample * cp.sum(si_var) <= i_state_ub]
        return lambda_var, gamma_matrix, theta_matrix, si_var, constraint
